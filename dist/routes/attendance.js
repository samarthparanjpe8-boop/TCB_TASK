...
attendanceRouter.get("/", asyncHandler(async (req, res) => {
    ...
    if (u.role === "teacher") {
        ...
        const q = { courseId, deletedAt: null };
        if (from && to) {
    ...
    const q = { courseId, studentId: u._id, deletedAt: null };
    if (from && to) {
        q.date = { $gte: utcDayStart(from), $lte: utcDayStart(to) };
    }
    const list = await Attendance.find(q).sort({ date: -1 }).lean();
...
attendanceRouter.post("/", requireTeacher, asyncHandler(async (req, res) => {
    const u = req.authUser;
    const courseId = parseObjectId(req.params.courseId, "courseId");
    await getCourseForTeacherOrThrow(courseId, u);
    ...
    const doc = await Attendance.findOneAndUpdate({ courseId, studentId: sid, date: day }, {
        $set: {
            status,
            recordedBy: u._id,
            deletedAt: null,
...
attendanceRouter.patch("/:attendanceId", requireTeacher, asyncHandler(async (req, res) => {
    const u = req.authUser;
    const courseId = parseObjectId(req.params.courseId, "courseId");
    await getCourseForTeacherOrThrow(courseId, u);
...
attendanceRouter.delete("/:attendanceId", requireTeacher, asyncHandler(async (req, res) => {
    const u = req.authUser;
    const courseId = parseObjectId(req.params.courseId, "courseId");
    await getCourseForTeacherOrThrow(courseId, u);
    const aid = parseObjectId(req.params.attendanceId, "attendanceId");
