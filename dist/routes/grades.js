...
import { getCourseForTeacherOrThrow, assertStudentCanViewCourse, assertStudentEnrolled, } from "../services/courseAccess.js";
export const gradesRouter = Router({ mergeParams: true });
gradesRouter.get("/", asyncHandler(async (req, res) => {
...
gradesRouter.post("/", requireTeacher, asyncHandler(async (req, res) => {
    const u = req.authUser;
    const courseId = parseObjectId(req.params.courseId, "courseId");
    await getCourseForTeacherOrThrow(courseId, u);
    ...
    if (score < 0 || score > max)
        throw new HttpError(400, "score must be between 0 and maxScore");
...
gradesRouter.patch("/:gradeId", requireTeacher, asyncHandler(async (req, res) => {
    const u = req.authUser;
    const courseId = parseObjectId(req.params.courseId, "courseId");
    await getCourseForTeacherOrThrow(courseId, u);
    ...
    const { assignmentName, score, maxScore } = req.body;
    if (assignmentName !== undefined)
        g.assignmentName = assignmentName.trim();
    if (typeof maxScore === "number" && maxScore > 0)
        g.maxScore = maxScore;
    if (typeof score === "number") {
        if (score < 0 || score > g.maxScore)
            throw new HttpError(400, "score must be between 0 and maxScore");
        g.score = score;
    }
    await g.save();
...
gradesRouter.delete("/:gradeId", requireTeacher, asyncHandler(async (req, res) => {
    const u = req.authUser;
    const courseId = parseObjectId(req.params.courseId, "courseId");
    await getCourseForTeacherOrThrow(courseId, u);
    const gradeId = parseObjectId(req.params.gradeId, "gradeId");
