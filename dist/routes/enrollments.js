...
enrollmentsRouter.get("/", asyncHandler(async (req, res) => {
...
enrollmentsRouter.post("/", requireTeacher, asyncHandler(async (req, res) => {
    const u = req.authUser;
    const courseId = parseObjectId(req.params.courseId, "courseId");
    await getCourseForTeacherOrThrow(courseId, u);
    ...
    const created = await Enrollment.findOneAndUpdate({ courseId, studentId: sid }, { $setOnInsert: { status: "active" }, $set: { status: "active" } }, { upsert: true, new: true, setDefaultsOnInsert: true });
    res.status(201).json({
...
enrollmentsRouter.delete("/:enrollmentId", requireTeacher, asyncHandler(async (req, res) => {
    const u = req.authUser;
    const courseId = parseObjectId(req.params.courseId, "courseId");
    await getCourseForTeacherOrThrow(courseId, u);
